...
function feature_product_list(results){
    ...
    if (response.status[0].Show_Status == "1") {
        ...
        while (i < response.products_list.length) {
            ...
            featured_pro += '<li class="item"> <a class="product-image" onclick="getProDetail('+pid+','+stock_status+',\''+ptype+'\')"><img src="' + response.products_list[i].image + '"/><span class="productimghover"></span></a><div class="sku">E13</div><h3 class="product-name"><a >' + response.products_list[i].name + '</a></h3><div class="hover-action"><div class="cartactions"><button  class="button btn-cart" title="Add to Cart" type="button"><span><span>Add to Cart</span></span></button></div></div></li>';
